...
def homepage(request):
    ...
    # Only run user-related queries if the user is authenticated.
    if request.user.is_authenticated:
        ...
        upvotes_received = Vote.objects.filter(post__author=request.user, value=Vote.UPVOTE).count()
    else:
        user_bookmarks = []  # empty list for anonymous users
        post_count = 0
        comment_count = 0
        upvotes_received = 0

    # Process tags from posts (splitting by comma and stripping whitespace)
    tags = Post.objects.values_list('tags', flat=True)
    unique_tags = set(tag.strip() for tag_list in tags for tag in tag_list.split(',') if tag.strip())
...
def post_detail(request, post_id):
    ...
    # Check authentication before querying for bookmarks
    if request.user.is_authenticated:
...
def user_profile(request, username):
    ...
    context = {
        ...
        'profile_created_at': user.profile.created_at,
    }
